...
async def welcome(request:Request , project_id: str,file: UploadFile, 
    ...
    asset_record = await asset_model.create_asset(asset=asset_resouce)
...
async def process_endpoint(request:Request, project_id: str, process_request: ProcessRequest):
    ...
    if process_request.file_id :
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal" : ResponseMessage.FILE_ID_ERROR.value
                }
        )
    ...
    if len(project_files_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "signal" : ResponseMessage.No_FILE_ERROR.value
            }
    )
    ...
    return JSONResponse(
        content={
            "signal" : ResponseMessage.PROCESSING_SUCCESS.value,
